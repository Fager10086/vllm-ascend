...
def split_qkv_rmsnorm_mrope_kernel(
    ...
    k_output_step_per_vec: tl.constexpr,
    k_output_step_per_iter_per_vec: tl.constexpr,
    v_batch_size_per_iter_per_vec: tl.constexpr,
    v_iter_num_per_vec: tl.constexpr,
    v_output_step_per_iter_per_vec: tl.constexpr,
    ...
    w_nmask_end: tl.constexpr,
    h_nmask_end_interleaved: tl.constexpr,
    w_nmask_end_interleaved: tl.constexpr,
):
    row_pid = tl.program_id(0)
    output_gm_q_offset = q_output_step_per_vec * row_pid
    output_gm_kv_offset = k_output_step_per_vec * row_pid
    ...
    output_q_indices = output_gm_q_offset + tl.arange(0, q_output_step_per_iter_per_vec)
    output_kv_indices = output_gm_kv_offset + tl.arange(0, k_output_step_per_iter_per_vec)
    ...
    output_q_offset_end = min(output_gm_q_offset + q_output_step_per_vec, q_total_ele_num)
    output_kv_offset_end = min(output_gm_kv_offset + k_output_step_per_vec, kv_total_ele_num)
    ...
    t_nblk_idx = tl.arange(0, half_rope_dim)
    h_nblk_idx = t_nblk_idx
    w_nblk_idx = t_nblk_idx
    ...
    if is_interleaved:
       ...
       t_nmask = cos_sin_nblk_idx < mrope_section_t
       h_nmask = (mrope_section_t - 1 < cos_sin_nblk_idx) & (cos_sin_nblk_idx < h_nmask_end)
       w_nmask = (h_nmask_end - 1 < cos_sin_nblk_idx) & (cos_sin_nblk_idx < w_nmask_end)
    ...
    for index in range(iter_num_per_vec):
        ...
        tl.static_print("idx:",idx)
        values_tmp = tl.load(in_qkv_gm_ptr + idx, mask=mask).reshape(qk_head_nums_per_iter_per_vec, head_size)
        ...
        t_idx = (t_mblk_idx + mblk_idx_offset)[:, None] + t_nblk_idx[None, :]
        thw_tensor = tl.load(cos_sin_ptr + t_idx, mask=t_mask, other=0)

        h_mmask = (h_mblk_idx + mblk_idx_offset) < h_cos_sin_batch_offset_end
        h_mask = (h_mmask[:, None]) & (h_nmask[None, :])
        h_idx = (h_mblk_idx + mblk_idx_offset)[:, None] + h_nblk_idx[None, :]
        thw_tensor += tl.load(cos_sin_ptr + h_idx, mask=h_mask, other=0)

        w_mmask = (w_mblk_idx + mblk_idx_offset) < w_cos_sin_batch_offset_end
        w_mask = (w_mmask[:, None]) & (w_nmask[None, :])
        w_idx = (w_mblk_idx + mblk_idx_offset)[:, None] + w_nblk_idx[None, :]
        thw_tensor += tl.load(cos_sin_ptr + w_idx, mask=w_mask, other=0)
        ...
        # ## compute ##
        # # q-rmsnorm
        normalized_values = values_tmp.to(tl.float32)
        normalized_values = normalized_values * normalized_values
        normalized_values = tl.sum(normalized_values, axis=1) / head_size
        normalized_values = values_tmp / tl.sqrt(normalized_values + eps).reshape(qk_head_nums_per_iter_per_vec, 1)
        ...
        if IS_PARTIAL_ROPE:
             ...
             tl.store(
                 out_q_ptr + output_q_indices + q_output_offset,
                 normalized_values_tmp.reshape(q_output_step_per_iter_per_vec),
             ...
             tl.store(
                 out_q_ptr + output_q_indices + q_output_offset,
                 values_tmp.reshape(q_output_step_per_iter_per_vec),
                 mask=output_q_indices  + q_output_offset < output_q_offset_end,
             )
        ...
        k_output_offset = index * k_output_step_per_iter_per_vec
        if IS_PARTIAL_ROPE:
            x1 = tl.extract_slice(
                 normalized_values_tmp,
                 offsets=(0, 0, 0),
            ...
            tl.store(
                 out_k_ptr + output_kv_indices + k_output_offset,
                 normalized_values_tmp.to(tl.bfloat16).reshape(k_output_step_per_iter_per_vec),
                 mask=output_kv_indices + k_output_offset < output_kv_offset_end,
             )
        else:
            values_tmp += normalized_values_tmp * cos_tensor
            tl.store(
                 out_k_ptr + output_kv_indices + k_output_offset,
                 values_tmp.to(tl.bfloat16).reshape(k_output_step_per_iter_per_vec),
                 mask=output_kv_indices + k_output_offset < output_kv_offset_end,
             )
...
#     output_v_indices = (output_gm_kv_offset + tl.arange(0, v_output_step_per_iter_per_vec))
    ...
    for _ in tl.range(v_iter_num_per_vec):
        ...
        # values = tl.load(in_qkv_gm_ptr + idx, mask=mask).reshape(v_output_step_per_iter_per_vec)
        # output_v_indices = output_mblk_idx[:, None] * total_hidden_size + nblk_idx[None, :]
        ...
        # output_mblk_idx += v_batch_size_per_iter_per_vec
        # output_v_indices += v_output_step_per_iter_per_vec 
...
def triton_split_qkv_rmsnorm_mrope(
    ...
    q_total_ele_num = batch_size * q_size
    kv_total_ele_num = batch_size * kv_size
...
# x: x * num_q_heads * rope_dim * 2 +  x * num_q_heads * rope_dim
    ...
    batch_size_per_iter_per_vec = 85*1024/qkv.element_size()//(max((num_q_heads+num_kv_heads)*head_size, num_q_heads*rope_dim*3) + rope_dim*5 + (
    ...
    q_output_step_per_vec = batch_size_per_vec * q_size
    q_output_step_per_iter_per_vec = batch_size_per_iter_per_vec * q_size
    k_output_step_per_iter_per_vec = batch_size_per_iter_per_vec * kv_size
    k_output_step_per_vec = batch_size_per_vec * kv_size
    ...
    v_output_step_per_iter_per_vec = v_batch_size_per_iter_per_vec * kv_size
    ...
    split_qkv_rmsnorm_mrope_kernel[grid](
        ...
        int(k_output_step_per_vec),
        int(k_output_step_per_iter_per_vec),
        int(v_batch_size_per_iter_per_vec), 
        int(v_iter_num_per_vec),
        int(v_output_step_per_iter_per_vec),
