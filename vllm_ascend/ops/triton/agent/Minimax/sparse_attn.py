...
def _bitonic_merge(
    ...
    if order == 2:
        ...
        flip = tl.full(x.shape, 1 if order else 0, tl.int32)
...
@triton.jit
def _topk_index_pair_merge_kernel(
    src_score_ptr,  # [NUM_TOPK_CHUNKS, num_idx_heads, total_q, T]
    src_idx_ptr,  # 1-indexed global block ids, 0 = invalid
    dst_score_ptr,  # same layout; only the first half of chunks is written
    dst_idx_ptr,
    stride_c,
    stride_h,
    stride_b,
    stride_t,
    BLOCK_SIZE_T: tl.constexpr,
    BLOCK_SIZE_K: tl.constexpr,  # 2 * BLOCK_SIZE_T
    USE_PDL: tl.constexpr,
):
    """Merge the top-T candidates from two chunks into one top-T list.

    The previous implementation sorted NUM_TOPK_CHUNKS * T values in one
    program.  On Ascend that grows the bitonic temporary tensors and UB demand
    linearly with NUM_TOPK_CHUNKS.  This kernel keeps the vector fixed at
    2 * T, and the Python wrapper forms the same global top-k through a binary
    reduction tree.
    """
    tl.static_assert(BLOCK_SIZE_K == 2 * BLOCK_SIZE_T)
    ...
    src_chunk = pid_out_chunk * 2 + off // BLOCK_SIZE_T
    in_chunk_offset = off % BLOCK_SIZE_T

    src_offset = (
        src_chunk * stride_c
        + pid_h * stride_h
        + pid_b * stride_b
        + in_chunk_offset * stride_t
    )
    score = tl.load(src_score_ptr + src_offset).to(tl.float32)
    score = tl.where(score != score, -1e30, score)
    idx = tl.load(src_idx_ptr + src_offset).to(tl.int32)
    ...
    extract_mask = tl.arange(0, 2) == 0
    merged_score = tl.sum(
        extract_mask[:, None] * tl.reshape(score, [2, BLOCK_SIZE_T]), axis=0
    )
    merged_idx = tl.sum(
        extract_mask[:, None] * tl.reshape(idx, [2, BLOCK_SIZE_T]), axis=0
    )

    dst_offset = (
        pid_out_chunk * stride_c
        + pid_h * stride_h
        + pid_b * stride_b
        + tl.arange(0, BLOCK_SIZE_T) * stride_t
    )
    tl.store(dst_score_ptr + dst_offset, merged_score)
    tl.store(dst_idx_ptr + dst_offset, merged_idx)


@triton.jit(do_not_specialize=["decode_query_len"])
def _topk_index_finalize_kernel(
    src_idx_ptr,  # [NUM_TOPK_CHUNKS, num_idx_heads, total_q, T], final result in chunk 0
    ...
    """Convert the final 1-indexed top-k list to the public 0-indexed output."""
    pid_b = tl.program_id(0)
    pid_h = tl.program_id(1)
    ...
    final_idx = tl.where(off_t < tl.minimum(topk, num_blocks), final_idx - 1, -1)
    ...
    tl.store(
        tif_ptrs,
        final_idx.to(ti_final_ptr.dtype.element_ty),
        mask=off_t < topk,
    )
...
def minimax_m3_index_decode(
    ...
    # Hierarchical two-way merge.  Every launch sorts only 2 * block_size_t
    # candidates, instead of a single NUM_TOPK_CHUNKS * block_size_t vector.
    # num_topk_chunks is always a power of two, so the final list ends in
    # chunk 0 of the active source buffer.
    src_score = topk_score_partial
    src_idx = topk_idx_partial
    if num_topk_chunks > 1:
        dst_score = torch.empty_like(topk_score_partial)
        dst_idx = torch.empty_like(topk_idx_partial)
        active_chunks = num_topk_chunks
        while active_chunks > 1:
            next_chunks = active_chunks // 2
            _topk_index_pair_merge_kernel[
                (batch, num_idx_heads, next_chunks)
            ](
                src_score,
                src_idx,
                dst_score,
                ...
                src_score.stride(3),
                BLOCK_SIZE_T=block_size_t,
                BLOCK_SIZE_K=2 * block_size_t,
                num_warps=2,
                num_stages=2,
                USE_PDL=use_pdl,
                **pdl_launch,
            )
            src_score, dst_score = dst_score, src_score
            src_idx, dst_idx = dst_idx, src_idx
            active_chunks = next_chunks

    _topk_index_finalize_kernel[(batch, num_idx_heads)](
        src_idx,
        topk_idx,
        seq_lens,
        SPARSE_BLOCK_SIZE,
