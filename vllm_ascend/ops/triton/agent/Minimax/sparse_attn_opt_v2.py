...
_ASCEND_VECTORCORE_NUM: int | None = None


def _get_ascend_vectorcore_num() -> int:
    global _ASCEND_VECTORCORE_NUM
    if _ASCEND_VECTORCORE_NUM is None:
        try:
            ...
            init_device_properties_triton()
            _ASCEND_VECTORCORE_NUM = int(get_vectorcore_num())
        except Exception:
            import triton.runtime.driver as driver

            device = torch.npu.current_device()
            properties = driver.active.utils.get_device_properties(device)
            _ASCEND_VECTORCORE_NUM = int(properties["num_vectorcore"])
    return _ASCEND_VECTORCORE_NUM


def _sparse_attn_grid(
    device: torch.device, total_work: int, *, has_dot: bool
) -> tuple[int, ...]:
    if device.type == "npu":
        vectorcore_num = _get_ascend_vectorcore_num()
        if has_dot:
            return (vectorcore_num // 2,)
...
if current_platform.is_cuda():
    ...
    @triton.jit
    def _sparse_attn_pdl_wait(USE_PDL: tl.constexpr):
        if USE_PDL:
            tl.extra.cuda.gdc_wait()

    @triton.jit
    def _sparse_attn_pdl_launch_dependents(USE_PDL: tl.constexpr):
        if USE_PDL:
...
def _gqa_sparse_decode_kernel(
    ...
    _sparse_attn_pdl_launch_dependents(USE_PDL)
...
@triton.jit(do_not_specialize=["total_work"])
def _merge_topk_attn_out_kernel(
    ...
    off_d = tl.arange(0, BLOCK_SIZE_D)
    for work_idx in range(worker_id, total_work, num_workers):
        pid_b = work_idx % total_q
        pid_h = work_idx // total_q
        ...
        lse_ptrs = (
            lse_ptr + pid_b * stride_l_b + pid_h * stride_l_h + off_c * stride_l_c
        )
...
def minimax_m3_sparse_attn(
    ...
    topk = min(topk_idx.shape[-1], block_table.shape[1])
...
def minimax_m3_sparse_attn_decode(
    ...
    max_topk = min(topk_idx.shape[-1], block_table.shape[1])
    gqa_group_size = num_heads // num_kv_heads
    use_fp8 = kv_cache.dtype in (torch.float8_e4m3fn, torch.float8_e5m2)
    is_arch_support_pdl = current_platform.is_arch_support_pdl
    use_pdl = is_arch_support_pdl() if callable(is_arch_support_pdl) else False
    ...
    if num_topk_chunks == 1:
        ...
        stride_o_c = 0
        stride_o_b = o_partial.stride(0)
        stride_o_h = o_partial.stride(1)
        stride_o_d = o_partial.stride(2)
    else:
    ...
    if num_topk_chunks > 1:
        merge_total_work = total_q * num_heads
        merge_grid = _sparse_attn_grid(output.device, merge_total_work, has_dot=False)
