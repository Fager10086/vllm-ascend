...
@triton.jit(do_not_specialize=["T", "total_blocks", "blocks_per_batch", "n_seqs"])
def chunk_local_cumsum_scalar_kernel(
    ...
    for work_idx in range(pid, total_blocks, num_programs):
        if IS_VARLEN:
            # Linear search chunk_offsets to find which sequence this work_idx belongs to
            i_s = 0
            for _s in range(n_seqs):
                start_off = tl.load(chunk_offsets + _s).to(tl.int32)
                if start_off <= work_idx:
                    i_s = _s
            seq_start_block = tl.load(chunk_offsets + i_s).to(tl.int32)
            i_block = work_idx - seq_start_block
        ...
        if HEAD_FIRST:
            ...
            b_s = tl.load(base_s + offsets, mask=mask, other=0.0).to(tl.float32)
...
def chunk_local_cumsum_scalar(
    ...
    if cu_seqlens is not None:
        # Build lightweight chunk_offsets on device (n_seqs+1 elements) instead of full chunk_indices
        lens = cu_seqlens[1:] - cu_seqlens[:-1]
        n_chunks_per_seq = (lens + OPTIM_BLOCK_SIZE - 1) // OPTIM_BLOCK_SIZE
        chunk_offsets = torch.nn.functional.pad(n_chunks_per_seq.cumsum(0), (1, 0), value=0).to(dtype=torch.int32)
        total_blocks = int(chunk_offsets[-1].item())
        n_seqs = len(lens)
    else:
        chunk_offsets = None
        num_blocks = triton.cdiv(T, OPTIM_BLOCK_SIZE)
        total_blocks = B * num_blocks
        n_seqs = 0
    blocks_per_batch = triton.cdiv(T, OPTIM_BLOCK_SIZE) if cu_seqlens is None else 0
