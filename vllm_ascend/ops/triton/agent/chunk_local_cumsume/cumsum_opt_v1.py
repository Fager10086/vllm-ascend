...
def chunk_local_cumsum_scalar(
    ...
    if use_varlen and len(cu_seqlens) == 2:
        use_varlen = False
        T = int(cu_seqlens[-1].item()) - int(cu_seqlens[0].item())
        cu_seqlens = None

    block_indices = prepare_chunk_indices(cu_seqlens, chunk_size=OPTIM_BLOCK_SIZE) if use_varlen else None
    num_blocks = len(block_indices) if use_varlen else triton.cdiv(T, OPTIM_BLOCK_SIZE)
...
if __name__ == "__main__":
    ...
    from vllm_ascend.ops.triton.fla.cumsum import chunk_local_cumsum as _chunk_local_cumsum
    chunk_local_cumsum = _chunk_local_cumsum
