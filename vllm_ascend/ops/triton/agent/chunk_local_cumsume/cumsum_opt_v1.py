...
@triton.jit
def chunk_local_cumsum_fast_kernel(
    s,
    o,
    scale,
    ...
    pid = tl.program_id(0)
    base = pid * SEG_LEN * H_TOTAL
    t_idx = tl.arange(0, SEG_LEN)
    h_idx = tl.arange(0, H_TOTAL)
    offsets = t_idx[:, None] * H_TOTAL + h_idx[None, :]
    b_s = tl.load(s + base + offsets).to(tl.float32)
    b_o = tl.cumsum(b_s, axis=0, reverse=REVERSE)
    if not REVERSE:
        last = tl.extract_slice(b_o, [SEG_LEN - 1, 0], [1, H_TOTAL], [1, 1])
    else:
        last = tl.extract_slice(b_o, [0, 0], [1, H_TOTAL], [1, 1])
    last_segs = tl.reshape(last, (N_SEGS, H_SEG))
    cum = tl.cumsum(last_segs, axis=0)
    exclusive = cum - last_segs
    correction = tl.reshape(exclusive, (1, H_TOTAL))
    b_o = b_o + correction
    if HAS_SCALE:
        b_o *= scale
    tl.store(o + base + offsets, b_o.to(s.dtype.element_ty))
    return
...
def chunk_local_cumsum_scalar(
    ...
    if not head_first and cu_seqlens is None and T % chunk_size == 0 and chunk_size >= 4:
        num_chunks = T // chunk_size
        NUM_PROGS = min(4, num_chunks)
        while num_chunks % NUM_PROGS != 0:
            NUM_PROGS -= 1
        chunks_per_prog = num_chunks // NUM_PROGS
        N_SEGS = 4
        SEG_LEN = chunk_size // N_SEGS
        H_SEG = chunks_per_prog * H
        H_TOTAL = N_SEGS * H_SEG
        g_6d = g.reshape(B, NUM_PROGS, chunks_per_prog, N_SEGS, SEG_LEN, H)
        g_perm = g_6d.permute(0, 1, 4, 3, 2, 5).contiguous()
        g_flat = g_perm.reshape(B * NUM_PROGS, SEG_LEN, H_TOTAL)
        o_flat = torch.empty_like(g_flat, dtype=output_dtype or g.dtype)
        grid = (B * NUM_PROGS,)
        chunk_local_cumsum_fast_kernel[grid](
            s=g_flat,
            o=o_flat,
            scale=scale,
            SEG_LEN=SEG_LEN,
        ...
        o_perm = o_flat.reshape(B, NUM_PROGS, SEG_LEN, N_SEGS, chunks_per_prog, H)
        o_6d = o_perm.permute(0, 1, 4, 3, 2, 5).contiguous()
        return o_6d.reshape(B, T, H)
