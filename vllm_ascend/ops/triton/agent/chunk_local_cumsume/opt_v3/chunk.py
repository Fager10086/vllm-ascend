...
def chunk_gated_delta_rule_fwd(
    ...
    # Pre-transpose once for all downstream kernels: (B, T, H) -> (B, H, T)
    g_t = g.transpose(1, 2).contiguous()
    beta_t = beta.transpose(1, 2).contiguous()
    # obtain WY representation. u is actually the new v.
    A = chunk_scaled_dot_kkt_fwd(k=k, beta=beta_t, g_cumsum=g_t, cu_seqlens=cu_seqlens, output_dtype=torch.float32)
