...
def chunk_scaled_dot_kkt_fwd_kernel(
    k,
    beta,  # [H, B, T] contiguous
    g_cumsum,  # [H, B, T] contiguous
    A,
    cu_seqlens,
    chunk_indices,
    T,
    ...
    for task_idx in range(i_task, NUM_TASKS, tl.num_programs(0)):
        i_t_i = task_idx % NT
        i_bh = task_idx // NT
        i_b = i_bh // H
        i_h = i_bh % H
...
def chunk_scaled_dot_kkt_fwd(
    ...
    grid_size = min(NUM_TASKS, core_num)

    chunk_scaled_dot_kkt_fwd_kernel[(grid_size,)](
        k=k,
        beta=torch.permute(beta, (2, 0, 1)).contiguous(),
        g_cumsum=torch.permute(g_cumsum, (2, 0, 1)).contiguous() if g_cumsum is not None else None,
        A=A,
