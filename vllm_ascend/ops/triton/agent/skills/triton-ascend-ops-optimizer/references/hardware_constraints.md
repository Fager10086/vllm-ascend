...
本文档详细说明昇腾 910B NPU 上 Triton 类算子开发的硬件约束。在分析 UB 占用、设计分核策略时参考此文档。
...
物理核心数量有限，决定了可并行执行的 program ID 数量。

### 2.2 分核原则

- **负载均衡**：将输入数据尽量均匀分配给各物理核
- **逻辑一致**：每个 program ID 对应的 kernel 处理逻辑相同
- **避免过度细分**：合理控制 Grid 大小，**禁止超过 NPU 物理核心数
- **循环补偿**：当分核数超过可用核心数时，在 kernel 内部通过循环处理

### 2.3 典型分核模式

对于纯 Vector 算子，典型分核模式如下：
...
对于包含 Cube 计算（即 Triton 算子内使用了 `tl.dot` ）的算子，典型分核模式如下：
