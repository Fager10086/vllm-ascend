...
## 8. 硬编码维度导致越界访存（静默精度错误）

**症状**：kernel 在某些参数组合下输出 `nan`、`inf` 或极大值（如 `3.86e+19`），在其他参数组合下正常。无 segfault，无编译错误，错误完全静默。

**关键排查**：
1. 检查 kernel 中 `tl.load` 的数量是否依赖某个"维度参数"（如 conv width、head 数）
2. 该参数是否被硬编码为固定值
3. 当实际值小于硬编码值时，多余的 `tl.load` 是否会越界

**根因**：Ascend NPU 上 `tl.load` 越界不报错，返回未定义值（通常极大或 nan），这些值参与后续计算（如 MAC + SiLU）后传播为 nan。

**修复**：将该维度参数声明为 `tl.constexpr`，用编译期 `if WIDTH == 2: ... elif WIDTH == 3: ...` 分支控制 load 数量。

**测试覆盖要求**：必须覆盖所有可能的维度参数值，不能只测试最常用值。

---

## 9. transpose().contiguous() 创建独立副本导致 in-place 更新丢失

**症状**：单步调用 output 正确，但多步推理时结果逐步偏离 reference。conv_state 未被更新。

**根因**：
```python
conv_state_t = conv_state.transpose(1, 2).contiguous()
# conv_state_t 是独立副本，非 view
# kernel 修改 conv_state_t，但 conv_state 未变
```

**修复**：kernel 执行后显式 copy 回原始 tensor：
```python
conv_state.transpose(1, 2).copy_(conv_state_t)
```

**注意**：此 copy 操作产生额外 Transpose + Copy 开销（~20us），但为正确性所必需。

---

## 10. strided 访存导致 60-85x 性能退化

**症状**：kernel Task Duration 从 ~17us 跳至 ~1440us，无编译错误。

**根因**：kernel 的主要访问维度（BLOCK_N 维度）在 tensor 中不连续（stride > 1）。Ascend NPU vector 单元对 stride > 1 的访问模式使用 gather/scatter 指令，效率极低。

**排查**：检查传入 kernel 的 tensor strides，确认 BLOCK_N 对应维度的 stride 是否为 1。

**解决**：在 wrapper 层做 transpose + contiguous 确保 BLOCK_N 维度 stride=1，宁可付出 ~13us 的 transpose 代价。

---

## 11. Triton tensor 不支持 slice 索引

**症状**：`ValueError('unsupported tensor index: slice(...)')`

**根因**：`tensor[:, k:k+1]` 或 `tensor[:, 0:state_len]` 等 slice 语法在 Triton kernel 中不支持。

**替代方案**：用显式 pointer offset + stride 逐元素或逐行访问。

---

## 12. 性能测试与正确性测试的 tensor layout 不一致

**症状**：正确性测试全部通过，但性能测试中 wrapper 层的 transpose 行为不同（多出一次实际内存拷贝），或反之。

**根因**：测试中 `torch.randn(N, state_len, dim).transpose(1, 2)` 创建的 tensor 已经是 dim-contiguous（因为原始数据 state_len 在前），而推理中 `torch.randn(N, dim, state_len)` 是 state-contiguous。两者的 `.transpose(1,2).contiguous()` 行为不同。

**解决**：性能测试的 tensor 创建方式必须与实际推理一致。检查模型代码中 conv_state 的分配方式。
