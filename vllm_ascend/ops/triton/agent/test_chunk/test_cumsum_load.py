...
def test_accuracy_chunk_varlen_with_model(
    H: int,
    D: int,
    mask_p: float,
    cu_seqlens: list[int],
    ...
    case_dirs = sorted(
        [
            os.path.join(base_dump_dir, d)
            for d in os.listdir(base_dump_dir)
            if os.path.isdir(os.path.join(base_dump_dir, d))
        ]
    )
    for case_dir in case_dirs:
        print(f"\n{'='*80}")
        print(f"Running case: {os.path.basename(case_dir)}")
        print(f"{'='*80}")
        ...
        k = inp["k"].to(device)
        v = inp["v"].to(device)
        beta = inp["beta"].to(device)
        g = inp["g"].to(device)
        h0 = inp["h0"].to(device)
        cu_seqlens = inp["cu_seqlens"].to(device)
        
        golden_g_out = out["g_out"].to(device)
        golden_o = out["o"].to(device)
        golden_A = out["A"].to(device)
        golden_final_state = out["final_state"].to(device)
        golden_w = out["w"].to(device)
        golden_h = out["h"].to(device)
        golden_v_new = out["v_new"].to(device)
        ...
        print_diff("g_out", golden_g_out, g_out, 0.005)
        print_diff("o", o, golden_o, 0.005)

        print_diff("A", golden_A, A, 0.005)
        print_diff("final_state", golden_final_state, final_state, 0.005)
        
        print_diff("w", golden_w, w, 0.005)
        print_diff("v_new", golden_v_new, v_new, 0.005)
        print_diff("h", golden_h, h, 0.005)
        
        assert_close("g_out", golden_g_out, g_out, 0.005)
        assert_close("o", o, golden_o, 0.005)

        assert_close("A", golden_A, A, 0.005)
        assert_close("final_state", golden_final_state, final_state, 0.005)
        
        assert_close("w", golden_w, w, 0.005)
        assert_close("v_new", golden_v_new, v_new, 0.005)
        assert_close("h", golden_h, h, 0.005)
