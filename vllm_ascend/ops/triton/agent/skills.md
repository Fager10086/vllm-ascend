...
## UB：
UB是vector核计算时的动态存储单元，每个kernel处理数据时，需要将数据从GM（Global Memory）通过tl.load方式读取到UB中，kernel操作UB中的数据，包括涉及到的中间变量统统都是在UB上存储的，当kernel计算完成，通过tl.store将输出从UB存回GB中。
...
- 尽量让每个kernel函数每次循环吃满UB，提升性能，在计算每次循环最大处理量时，用//而非tl.cdiv，否则容易出现ub overflow问题。
- 对于规约操作，最好按照最大的维度上进行规约操作，可提升性能。
- 对同一个GM地址，进行多次load，要对这几个load的tensor进行计算，比如加法，建议一次load后做一次加法，一次load后再做一次加法。因为统一load，再做加法后，计算流水会等待这些tensor全部load完再计算，效率较低。
- 如果有多个写入流，建议边计算边写，因为写流基本不冲突，计算完提前写入，增大并行的可能。
