...
def _get_cached_outputs(num_tokens, q_size, kv_size, device, dtype):
    ...
    _cache[cache_key] = (q_output, k_output, v_output)
    return q_output, k_output, v_output


def _get_launcher(block_dim):
    global _launcher_cache
    launcher = _launcher_cache.get(block_dim)
    if launcher is None:
        launcher = split_qkv_rmsnorm_mrope_kernel[(block_dim,)]
        _launcher_cache[block_dim] = launcher
    return launcher
...
def triton_split_qkv_rmsnorm_mrope(
    ...
    q_output, k_output, v_output = _get_cached_outputs(
        num_tokens, q_size, kv_size, qkv.device, qkv.dtype
    )

    launcher = _get_launcher(block_dim)
    launcher(
        ...
        q_bias is not None, is_interleaved, rope_dim, rope_dim // 2,
        rope_dim != head_size, q_size + 2 * kv_size,
    )
