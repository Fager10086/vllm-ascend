...
def split_qkv_rmsnorm_rope_kernel(
    ...
    for index in range(loop_num):
        ...
        # Load V and store immediately
        in_v = tl.load(in_base + q_hidden_size + kv_hidden_size + tl.arange(0, kv_hidden_size))
        tl.store(v_ptr + token_idx * kv_hidden_size + tl.arange(0, kv_hidden_size), in_v)

        # Load cos/sin via position index (row-by-row for discrete access)
        pos_idx = tl.load(pos_ptr + token_idx).to(tl.int64)
        cos_base = pos_idx * HEAD_DIM
        cos = tl.load(cos_sin_ptr + cos_base + tl.arange(0, HALF_HEAD_DIM)).reshape(1, HALF_HEAD_DIM)
        sin = tl.load(cos_sin_ptr + cos_base + tl.arange(HALF_HEAD_DIM, HEAD_DIM)).reshape(1, HALF_HEAD_DIM)
        ...
        # Q RoPE - compute on half-dims, assemble result
        ...
        rq1 = (q_x1 * cos - q_x2 * sin).to(tl.bfloat16)
        rq2 = (q_x2 * cos + q_x1 * sin).to(tl.bfloat16)
        roped_q = tl.zeros((NUM_Q_HEADS, HEAD_DIM), dtype=tl.bfloat16)
        roped_q = tl.insert_slice(roped_q, rq1, offsets=(0, 0), sizes=(NUM_Q_HEADS, HALF_HEAD_DIM), strides=(1, 1))
        roped_q = tl.insert_slice(roped_q, rq2, offsets=(0, HALF_HEAD_DIM), sizes=(NUM_Q_HEADS, HALF_HEAD_DIM), strides=(1, 1))

        # Store Q immediately (edge compute edge store)
        tl.store(q_ptr + token_idx * q_hidden_size + tl.arange(0, q_hidden_size), roped_q.reshape(q_hidden_size))
        ...
        rk1 = (k_x1 * cos - k_x2 * sin).to(tl.bfloat16)
        rk2 = (k_x2 * cos + k_x1 * sin).to(tl.bfloat16)
        roped_k = tl.zeros((NUM_KV_HEADS, HEAD_DIM), dtype=tl.bfloat16)
        roped_k = tl.insert_slice(roped_k, rk1, offsets=(0, 0), sizes=(NUM_KV_HEADS, HALF_HEAD_DIM), strides=(1, 1))
        roped_k = tl.insert_slice(roped_k, rk2, offsets=(0, HALF_HEAD_DIM), sizes=(NUM_KV_HEADS, HALF_HEAD_DIM), strides=(1, 1))

        # Store K
        tl.store(k_ptr + token_idx * kv_hidden_size + tl.arange(0, kv_hidden_size), roped_k.reshape(kv_hidden_size))
