...
def test_performance_with_given_shape():
    # 解析输入形状字符串
    # shape_str = "1,32764,1,128;1,32764,4,128;1,32764,4,128;1,4,32764"
    shape_str = "1,10288,2,128;1,10288,8,128;1,10288,8,128;1,8,10288"
    shapes = [list(map(int, s.split(','))) for s in shape_str.split(';')]
    # shapes[0] -> k: [1, 32764, 1, 128]
    # shapes[1] -> w: [1, 32764, 4, 128]
    # shapes[2] -> u: [1, 32764, 4, 128]
    # shapes[3] -> g: [1, 4, 32764]
    ...
    # 门控 g 原始形状为 [B, H, T]，需要转换为 [B, T, H] 以满足包装函数的要求
    g_raw = torch.randn(B, H, T, device=device, dtype=torch.float32)
    g = g_raw.transpose(1, 2)   # 现在形状 [B, T, H]
    # g = g.contiguous()
    # 可选的初始状态
    ...
    # 使用 triton.testing.do_bench 测量性能
    # print("Measuring performance...")
    # ms = triton.testing.do_bench(lambda: chunk_gated_delta_rule_fwd_h(
    #     k=k, w=w, u=u, g=g,
    #     initial_state=initial_state,
    #     output_final_state=True,
    #     chunk_size=chunk_size,
